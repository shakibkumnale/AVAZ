...
export default function Chatbots(props) {
  ...
  const Newchat = (event) => {
    ...
    let CHATS_DivUser = document.getElementById("chats");
    let lll=document.getElementsByClassName("USER")
    let lll2=document.getElementsByClassName("AVAZ")
    setVisiable("show all")
    for (let i = 0; i < lll.length; i++) {
        const element = lll[i];
        const element2 = lll2[i];
        element.style.display="none"
        element2.style.display="none"
        console.log(element);
        console.log(element2);
    }
    // CHATS_DivUser.removeChild(lll)
    console.log(lll+"lllllllllllllll");
    let eleU = document.createElement('div')
  ...
  const copyBTn_chatHis = (e) => {
    ...
    let  aa=navigator.clipboard.writeText(chat)
    alert(chat)
    // console.log(chat);
    // copy(chat)
